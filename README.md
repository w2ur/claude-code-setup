...
Entry points. You type these — they orchestrate everything else.
...
The workers. Each has a specific role, a specific model, and clear boundaries.

| Agent | Model | Memory | What it does | What it doesn't do |
|-------|-------|--------|--------------|--------------------|
| **implementer** | sonnet → opus | ✅ | Executes tasks with "done when" criteria | Architecture decisions |
| **architect** | opus | ✅ | Diagnoses structural problems, produces plans | Write production code |
| **portfolio-sync** | sonnet | — | Cross-repo coherence (manifests, JSON, docs) | Creative content (taglines, descriptions) |
| **docs-checker** | sonnet | — | Audits + fixes README, CLAUDE.md, URLs | Compliance standards |
| **portfolio-audit** | haiku | — | Read-only compliance check (signature, secrets, tests) | Fix anything |
| **dev-scanner** | sonnet | — | Discovers projects, detects drift, finds orphans | Modify files |
...
Preloaded knowledge, injected into agents at startup. Not tools — context.
...
All advisory. They remind — they never block.
...
This system was built for a very specific situation: one person who doesn't read code, 10+ apps, mixed stacks, zero budget. Your situation is probably different (and probably more reasonable).
...
- **Personal data.** Public apps are mentioned by name (they're live on the internet anyway). Private apps, personal URLs, and paths are anonymized. See `scripts/anonymization.yaml` for the mapping.
- **A universal solution.** This works for me. Parts of it will work for you. All of it, probably not — and that's fine.

## Maintenance

This repo stays in sync with my actual `~/.claude/` setup via a Python sync script that copies, anonymizes, and audits for data leaks. See `scripts/` for details.
...
*Built by [William](https://william.revah.paris) — someone who doesn't read code but ships 10+ apps anyway. Read [the philosophy](docs/philosophy.md) for the full story.*
