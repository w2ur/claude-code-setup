...
- **Four local crontab entries plus one launchd LaunchAgent, deliberately offset.** All are local, not cloud `/schedule` routines: cloud routines run in an isolated sandbox with no access to the local `~/Dev` filesystem, so they can't read repo state.
