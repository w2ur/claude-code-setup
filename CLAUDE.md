...
- **Five local crontab entries, deliberately offset.** All are local, not cloud `/schedule` routines: cloud routines run in an isolated sandbox with no access to the local `~/Dev` filesystem, so they can't read repo state.
  ...
  - **Daily, 7:07am** — `~/Dev/vigie/scripts/vigie-refresh.sh` → `~/.claude/vigie-cron.log`. Rebuilds the local control panel's snapshot and static site. **The only entry whose script lives in a project repo rather than `~/.claude/scripts/`**, because it builds that project; it is therefore not covered by `/sync-setup`. Guards on `node`/`npm`/`git` with a **fatal exit 2**, never a skip — a collector that cannot reach its tools would record every source unavailable and render an empty portfolio, which is indistinguishable from a real empty portfolio. It also **`cd`s into the repo first, deliberately**: Astro resolves its content-collection base against the cwd of `astro build`, and cron runs with cwd `$HOME`, so building from there would silently collect zero documents rather than failing.
